...
The third layer serves the purpose of **End to End** packet delivery,  *layer 2 does not use MAC addresses like layer 2 does, it uses a different addressing scheme called `IP address`* *by using a different addressing scheme then the one that layer 2 uses called `IP address`*. An IP Address is made up of 32 bits represented as 4 octets, each 0-255, and that address is used to represent a computer in the wide web of machines connected to the internet. 

A component used that belongs to this layer is the `Router`, the router lets computers communicate outside of their networks in-directly, passing the packets though and routing them to their end goal one router at a time.
...
--------------------------------------------------------------------------------------------
    ...
    1.  Packet after leaving C1, on it's way to the first router (R0).
...
The way layer 4 sorts the incoming `data streams` is by using it's own addressing scheme called `transport protocols & ports`(similar to ports on a switch). And it works like that:
