...
| [LDAP](#lighweight-directory-access-protocol-aka-ldap)                | Lighweight Directory Access protocol                          |
...
## Internet Control Message Protocol A.K.A [ICMP](https://www.urbandictionary.com/define.php?term=Spell%20iCup)
###### *[#Layer-3] [#Portless]*
...
- (I think that by local-link you mean LAN then itws not the physical network)
```
...
```diff
- (What IP address would the compute ruse untilit will get one from the DHCP server?)
```
...
> 
> NOTICE: NBT Names are apply only to the local area network, they are not to be confused with domain names.
...
```
Since at the time of the protocol's manifestation the cyber threat wasn't as potent, all communication over TelNet is sent as cleartext (Not encrypted). Given that, telnet has to be manually enabled with the newer systems nowadays.
...
> #### `Commands`
>
> Using a terminal the user can send and extract files from the conneted host. The commands and files are transferred on two different data streams.
...
```diff
- (What commends are avilable in TFTP?)
```
...
Overall, HTTP works as a request response type protocol, just like dns and dhcp. The whole point of it is to get the hosted webpage from the server, like "https://www.google.com" where I research this stuff or "jokes.moomoo.co.il" where I get my 'your mom jokes'.
...
![The Original Spacejam Website](https://https://www.themarysue.com/wp-content/uploads/2010/12/SpaceJam_blankdcr.png)
###### pretty simple right?
...
### WHY WAS IT INVENTED?
Smtp is the protocol used to send and forward mails to a mailbox on the mail server, but to view those emails interfacing with the server directly is required. No only do you need to go on the server but you also need both yout machine and the server to be connected. POP solves that issue by downloading you emails from the mail server for you, so you can access them from your machine and view them offline.
...
> #### `AUTHENTICATION`
>
> The client has to authenticate to the server using USER and PASS commands to pass their credentials. After authenticating correctly the server opens the client's mailbox and locks any access to it's content so that it won't change in the midst of a session.
> 
...
### HOW DOES IT WORK?
Communication with the protocol starts when the client sends a request and the server returns a reponse, initiating a connection. Then a session is created with a set of keys, one for encryption and one for signing messages, known to both sides.

`As it's transport layer used can be either TCP (on port 445) or NetBIOS over TCP (nbns on port 139), The protocol also requires authentication`. Either using NTLM or Kerberos (for domain purposes), the first one being vulnurable to attacks. After the authentication and checking permission for the shared folder the server will open a connection to the folder where the cleint can perform various operations, like read and write information from/to it, rename it, delete it, create a new file, copy, save it and more. All of these operations are done on the server side using commands sent by the client.
...
```diff
- (Stratesd checking this protocol but you wrote too much information summaries it)
```
...
> #### `Server` `Row Data`
>
> The server will answer `SQL Batch` messages that return the data that was requested stored in rows.
...
The LDAP protocol is used to enumerate and perform actions on the AD structure of a domain, with this protocol a client can communicate with the DC and gather information about the domain's structure.
...
This protocol works and a request-response/client-server model. using this protocol a client can perform many oprations on the AD structure as follows:

These messages all follow the same format each with their own MessageID.

> **Bind**
>
> First of all, a client must authenticate themselves to access the AD structure. A client must give a name and authentication method.
>
> **Bind Response**
>
...
> **Search**
>
> A client may search the Ad structure using queries, subject to their permissions and limitations.
>
...
> **Modify**
>
> This operation lets a client perform modifications of objects in the Active Directory. A client must specify an object and what changes he wants to perform.

> **Add**
>
> This operation lets a client add an object to the Active Directory structure, a client must spcify the full path of the object as the name and it's attributes.
...
> **Delete**
>
> This operation lets a client delete an object from the Active Directory by specifying it's full path.

> **Modify DN/Rename**
>
> This opreation lets a client change the DN (Distiguished Name) of an object in the AD structure, basically moving it in the structure (Since the DN also qualifies as the path). A client must provide the path to the object and the new DN.
>
> **Response**
>
...
```diff
- (In the Ethernet version What port an layer dos it use?)
```
